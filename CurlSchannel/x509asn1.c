 ...
 */
CURLcode Curl_ssl_push_certinfo_len(struct wintls* data,
    int certnum,
    const char* label,
    const char* value,
    size_t valuelen)
...
CURLcode Curl_extract_certinfo(struct wintls* tls,
    int certnum,
    const char* beg,
    const char* end)
{
    ...
    const char* beg, const char* end)
{
    ...
    case 1:
        ...
        infof(data, "  subjectAltName: %s matched", tls->dispname);
        return CURLE_OK;
    case 0:
        /* an alternative name field existed, but didn't match and then
           we MUST fail */
        infof(data, "  subjectAltName does not match %s", tls->dispname);
        return CURLE_PEER_FAILED_VERIFICATION;
    }
